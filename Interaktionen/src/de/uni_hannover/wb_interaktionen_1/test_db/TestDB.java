...
public class TestDB {
    ...
    /** Method to find the hall group for a specific person.
     *
     * @param personalID : ID of person to find the hall group for
     * @return The ID of the hall group the person is inside
     * @throws SQLException: : Thrown if user does not exist in DB,
     *                         In case of query failure due to external issues in DB or if
     *                         someone changed one of the column names in the DB
     * @author David Sasse
     * */
    public int findGroupFor(String personalID) throws SQLException {
        String sql = "SELECT * FROM " + USER_TABLE + " WHERE personalID = ?";
        PreparedStatement intermediate = dbConnection.prepareStatement(sql);
        intermediate.setString(1, personalID);
        ResultSet results = intermediate.executeQuery();
        ...
        while(users.next()) {
            if(users.getInt("roomID") == 0){
                ...
                res.add(new User(
                        ...
                        new HallGroup(users.getInt("groupID"), db)));
            }
        }
        return res;
    }
    ...
    public ArrayList<User> getAllUserInGroupAsUserList(HallGroup group) throws SQLException {
        ArrayList<User> result = new ArrayList<>();
        String sql = "SELECT * FROM " + USER_TABLE + " WHERE groupID = ?";
        PreparedStatement intermediate = dbConnection.prepareStatement(sql);
        intermediate.setInt(1, group.getId());
        ResultSet users = intermediate.executeQuery();
        while(users.next()) {
            result.add(new User(
                    users.getString("personalID"),
